...
class AddToCartTest extends CommonTest{
  ...
  */
  ...
  public function testAddToCartEventNotCreatedWhenS2SDisabled(){
    $this->fbeHelper->method('isS2SEnabled')->willReturn(false);
    $observer = new \Magento\Framework\Event\Observer(['eventId' => '1234']);
    $this->assertEquals($this->addToCartObserver->execute($observer), $this->addToCartObserver);
    $this->assertEquals(0, count($this->serverSideHelper->getTrackedEvents()));
  }

  public function testAddToCartEventCreatedWhenS2SEnabled(){
