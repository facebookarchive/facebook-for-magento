...
class InitiateCheckoutTest extends CommonTest{
  ...
  */
  ...
  public function testInitiateCheckoutEventNotCreatedWhenS2SDisabled(){
    $this->fbeHelper->method('isS2SEnabled')->willReturn(false);
    $observer = new \Magento\Framework\Event\Observer(['eventId' => '1234']);
    $this->assertEquals($this->initiateCheckoutObserver->execute($observer), $this->initiateCheckoutObserver);
    $this->assertEquals(0, count($this->serverSideHelper->getTrackedEvents()));
  }

  public function testInitiateCheckoutEventCreatedWhenS2SEnabled(){
