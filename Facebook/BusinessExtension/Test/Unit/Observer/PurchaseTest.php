...
class PurchaseTest extends CommonTest{
  ...
  */
  ...
  public function testPurchaseEventNotCreatedWhenS2SDisabled(){
    $this->fbeHelper->method('isS2SEnabled')->willReturn(false);
    $observer = new \Magento\Framework\Event\Observer(['eventId' => '1234']);
    $this->assertEquals($this->purchaseObserver->execute($observer), $this->purchaseObserver);
    $this->assertEquals(0, count($this->serverSideHelper->getTrackedEvents()));
  }

  public function testPurchaseEventCreatedWhenS2SEnabled(){
